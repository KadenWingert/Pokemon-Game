...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == ter_gate) // If it moves to a gate, generate a new map
  {
    //world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
    if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == world.cur_map->n)
    {
      world.cur_idx[dim_y]--;
      new_map(0);
      // world.pc.pos[dim_x] = world.cur_map->s;
      // world.pc.pos[dim_y] = MAP_Y-1;
    }
    else if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == world.cur_map->s)
    {
      world.cur_idx[dim_y]++;
      new_map(0);
    ...
    else if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == world.cur_map->e)
    {
      world.cur_idx[dim_x]++;
      new_map(0);
    ...
    else if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == world.cur_map->w)
    {
      world.cur_idx[dim_x]--;
      new_map(0);
      // world.pc.pos[dim_y] = world.cur_map->e;
      // world.pc.pos[dim_x] = MAP_X-1;
    }
    world.pc.pos[dim_y] = world.cur_idx[dim_y];
    world.pc.pos[dim_x] = world.cur_idx[dim_x];
return 1;
  }
...
void io_handle_input(pair_t dest)
{
  ...
  do
  {
    ...
    case 'f':
      ...
      {
        int x, y;
       // mvprintw(0, 0, "Enter the x and y coordinates that you would like to fly to:");
        refresh();
        scanw("%d %d", &x, &y);
        mvprintw(0, 0, "Flying to (%d,%d)", x, y);
        refresh();
        ...
        {
          mvprintw(0, 0, "You have entered invalid coordinates. Try again.");
          refresh();
        }
      }
      break;
