...
void game_loop()
{
  ...
  while (!world.quit)
  {
    ...
    c->next_turn += move_cost[n ? n->ctype : char_pc]
     ...
     //If it is the pc who has moved,they just moved a cell and were previously in tall grass, they have a 10% chance to encounter a pokemon
    if (p && (c->pos[dim_y] != d[dim_y] || c->pos[dim_x] != d[dim_x]) &&
        (world.cur_map->map[d[dim_y]][d[dim_x]] == ter_grass) &&
        (rand() % 100 < ENCOUNTER_PROB)){
    //     if (p && (world.cur_map->map[d[dim_y]][d[dim_x]] == ter_grass) && (rand() % 100 < ENCOUNTER_PROB))
    // {
      mvprintw(0,0,"YOU HAVE STUMBLED ACROSS A WILD POKEMON");
      refresh();
      getch();
