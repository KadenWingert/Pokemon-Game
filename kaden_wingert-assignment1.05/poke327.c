...
void battle()
{
  clear();
  move(1, 0);
  mvprintw(1, 0, "YOU ARE IN BATTLE");

  while (1)
  { // Makes it so you are unable to move the characters while viewing the trainer list
    int32_t key = getch();
    if (key == 27 || key == 'q')
    {
      break;
    ...
    {
      mvprintw(0, 0, "Invalid command. Press esc or q to go back to the game");
    }
  }
}
static void move_hiker_func(character_t *c, pair_t dest)
{
  if (c->defeated == 0)
  {
    if ((c->pos[dim_x] - 1 == world.pc.pos[dim_x] && c->pos[dim_y] - 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] - 1 == world.pc.pos[dim_x] && c->pos[dim_y] + 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] - 1 == world.pc.pos[dim_x] && c->pos[dim_y] + 0 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] + 0 == world.pc.pos[dim_x] && c->pos[dim_y] - 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] + 0 == world.pc.pos[dim_x] && c->pos[dim_y] + 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] + 1 == world.pc.pos[dim_x] && c->pos[dim_y] - 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] + 1 == world.pc.pos[dim_x] && c->pos[dim_y] + 1 == world.pc.pos[dim_y]) ||
        (c->pos[dim_x] + 1 == world.pc.pos[dim_x] && c->pos[dim_y] + 0 == world.pc.pos[dim_y]))
...
void trainer_List()
{
  ...
  int boundry = 1;
  clear();
  mvprintw(0, 0, "You are viewing the trainer list. Press esc to resume the game");
  for (int y = 0; y < MAP_Y; ++y)
  {
    for (int x = 0; x < MAP_X; ++x)
    {
      ...
      if (world.cur_map->cmap[y][x] != NULL)
      {
        num_trainers++;
        boundry++;
        int yC = world.cur_map->cmap[y][x]->pos[dim_y] - world.pc.pos[dim_y];
        int xC = world.cur_map->cmap[y][x]->pos[dim_x] - world.pc.pos[dim_x];

        mvprintw(num_trainers, 0, "%c: %d %s,%d %s\n", world.cur_map->cmap[y][x]->symbol,
                 abs(yC), yC >= 0 ? "south" : "north",
                 abs(xC), xC >= 0 ? "east" : "west");
      }
    }
  }
  int scroll = 1;
  ...
  { // Makes it so you are unable to move the characters while viewing the trainer list
        ...
        for (int y = 0; y < MAP_Y; ++y)
        {
          for (int x = 0; x < MAP_X; ++x)
          {
            if (world.cur_map->cmap[y][x] != NULL)
            {
              ...
              mvprintw(scroll + num_trainers, 0, "%c: %d %s,%d %s", world.cur_map->cmap[y][x]->symbol,
                       abs(yC), yC >= 0 ? "south" : "north",
                       abs(xC), xC >= 0 ? "east" : "west");
            }
          }
    ...
    {
      mvprintw(0, 0, "Invalid command. Press esc to go back to the game or up/down arrows to cursor");
    }
  }
}

void enter_pokeBuilding()
{
  clear();
  move(1, 0);
  mvprintw(1, 0, "YOU ARE IN A POKEBUILDING");
...
void game_loop()
{
  ...
  while (!quit)
  {
    ...
    else if (key == '7' || key == 'y')
    {
      ...
      c = heap_remove_min(&world.cur_map->turn);
      if (world.pc.pos[dim_x] - 1 > 0 && world.pc.pos[dim_y] - 1 > 0)
      {
        if (c == &world.pc)
        {
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1]] != INT_MAX)
          {
            ...
            character_t *tempc = world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1];
            if (tempc != NULL && tempc->defeated == 0 && tempc->pos[dim_y] == world.pc.pos[dim_y] - 1 && tempc->pos[dim_x] == world.pc.pos[dim_x] - 1)
            {
              battle();
              tempc->defeated = 1;
            }
    ...
    else if (key == '8' || key == 'k')
    {
      ...
      clrtoeol();
      mvprintw(0, 0, "UP");
      if (world.pc.pos[dim_x] + 0 > 0 && world.pc.pos[dim_y] - 1 > 0)
      {
        c = heap_remove_min(&world.cur_map->turn);

        if (c == &world.pc)
        {
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] + 0]] != INT_MAX)
          {
            character_t *tempc = world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] + 0];
            if (tempc != NULL && tempc->defeated == 0 && tempc->pos[dim_y] == world.pc.pos[dim_y] - 1 && tempc->pos[dim_x] == world.pc.pos[dim_x])
            {
              battle();
              tempc->defeated = 1;
            }
            ...
            {
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] + 0] = c;
              // c->pos[dim_x]--;
              c->pos[dim_y]--;
            }
          }
    ...
    else if (key == '9' || key == 'u')
    {
      ...
      if (world.pc.pos[dim_x] + 1 < 79 && world.pc.pos[dim_y] - 1 > 0)
      {
        c = heap_remove_min(&world.cur_map->turn);
    ...
    else if (key == '6' || key == 'l')
    {
      ...
      clrtoeol();
      mvprintw(0, 0, "RIGHT");
      if (world.pc.pos[dim_x] + 1 < 79 && world.pc.pos[dim_y] + 0 > 0)
      {
        c = heap_remove_min(&world.cur_map->turn);
        if (c == &world.pc)
        {
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] + 0][c->pos[dim_x] + 1]] != INT_MAX)
          {
            character_t *tempc = world.cur_map->cmap[c->pos[dim_y] + 0][c->pos[dim_x] + 1];
            if (tempc != NULL && tempc->defeated == 0 && tempc->pos[dim_y] == world.pc.pos[dim_y] && tempc->pos[dim_x] == world.pc.pos[dim_x] + 1)
            {
              battle();
              tempc->defeated = 1;
            }
            ...
            {
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              world.cur_map->cmap[c->pos[dim_y] + 0][c->pos[dim_x] + 1] = c;
              c->pos[dim_x]++;
              // c->pos[dim_y]--;
            }
          }
    ...
    else if (key == '3' || key == 'n')
    {
      ...
      if (world.pc.pos[dim_x] + 1 < 79 && world.pc.pos[dim_y] + 1 < 21)
      {
        c = heap_remove_min(&world.cur_map->turn);
        if (c == &world.pc)
        {
    ...
    else if (key == '2' || key == 'j')
    {
      ...
      clrtoeol();
      mvprintw(0, 0, "DOWN");
      if (world.pc.pos[dim_x] + 0 > 0 && world.pc.pos[dim_y] + 1 < 21)
      {
        c = heap_remove_min(&world.cur_map->turn);
        if (c == &world.pc)
        {
    ...
    else if (key == '1' || key == 'b')
    {
      ...
      if (world.pc.pos[dim_x] - 1 > 0 && world.pc.pos[dim_y] + 1 < 21)
      {
        c = heap_remove_min(&world.cur_map->turn);
        if (c == &world.pc)
        {
    ...
    else if (key == '4' || key == 'h')
    {
      ...
      clrtoeol();
      mvprintw(0, 0, "LEFT");
      if (world.pc.pos[dim_x] - 1 > 0 && world.pc.pos[dim_y] + 0 > 0)
      {
        c = heap_remove_min(&world.cur_map->turn);
        if (c == &world.pc)
        {
    ...
    else if (key == '5' || key == ' ')
    {
      move(0, 0);
      clrtoeol();
      mvprintw(0, 0, "SPACE OR 5, DO NOTHING");
      c = heap_remove_min(&world.cur_map->turn);
      if (c == &world.pc)
      {
        c->next_turn += move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]]];
      ...
      else
      {
        ...
        { // Nested loop to go through all trainers to see if they are on the same spot as the pc
          for (int x = 1; x < MAP_X - 1; ++x)
          {
            if (c->defeated == 0 && c->pos[dim_y] == world.pc.pos[dim_y] && c->pos[dim_x] == world.pc.pos[dim_x])
            {
              battle();
              if (c != &world.pc)
              {
    ...
    else
    {
      move(0, 0);
      clrtoeol();
      mvprintw(0, 0, "That is not a valid key try another, or 'q' to quit.");
    }
    // if (c == &world.pc)
    // {
    //   USED FOR DEBUGGING
    //    mvprintw(22, 0, "COST %d", move_cost[char_pc][world.cur_map->map[c->pos[dim_y] + 0][c->pos[dim_x] - 1]]);
    //    mvprintw(22, 20, "XPOS %d", c->pos[dim_x]);
    //    mvprintw(22, 28, "YPOS %d", c->pos[dim_y]);
    //    mvprintw(23, 20, "XPOS %d", world.pc.pos[dim_x]);
    //    mvprintw(23, 28, "YPOS %d",world.pc.pos[dim_y]);
    //   refresh();
    // }
    pathfind(world.cur_map);
  }
